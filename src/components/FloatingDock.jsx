...
const FloatingDock = memo(function FloatingDock() {
  ...
  const isActive = (path) => {
    // Special case for My Gallery route (dynamic username)
    if (path.includes('/gallery') && user) {
      return location.pathname === `/${user.username}/gallery`
    }
    // Special case for My Videos route (dynamic username)
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 px-5 py-3 rounded-full bg-light-navbar/95 dark:bg-dark-navbar/95 border border-light-navbar/30 dark:border-dark-navbar/30 backdrop-blur-md shadow-2xl transition-all duration-300">
      ...
      {/* My Gallery */}
      <Link to={`/${user.username}/gallery`} className={linkClass(`/${user.username}/gallery`)} title="My Gallery">
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* Videos */}
      <Link to="/video" className={linkClass('/video')} title="All Videos">
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* My Videos */}
      <Link to={`/${user.username}/video`} className={linkClass(`/${user.username}/video`)} title="My Videos">
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
